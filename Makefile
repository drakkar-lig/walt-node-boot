...
all: build/rpi-sd.dd.gz build/rpi-sd-files.tar.gz build/pc-usb.dd
...
# build/pc-usb.dd is the USB image to boot PC nodes
build/pc-usb.dd: .date_files/pc_boot_builder_image
	@mkdir -p build
	@docker run --rm --privileged -v /dev:/dev --entrypoint /root/entry_point.sh waltplatform/pc-boot-builder > build/pc-usb.dd
